...
$monthly_expense[1]["label"] = "groceries";
...
for ($count = 0; $count < 10; $count++) {
    ...
    $query = "SELECT `$label` FROM nonworking WHERE username='$uname'";
...
for ($count = 0; $count < 10; $count++) {
    ...
    $query = "SELECT `$label` FROM nonworking_expense WHERE userid='$uname'";
    $result = $con->query($query);
    if ($result) {
        $row = $result->fetch_assoc();
